...
from glyphNameFormatter.unicodeRangeNames import getRangeName

path = '/Users/frederik/Documents/dev/fontClubBelgica/fontpage-html/fonts/Pep/Web/lc/pep-solid-rgb-pro.woff2'

#https://github.com/justvanrossum/fontgoggles/blob/aad0aa0423ad16ca8bafb3b2493c22129b775566/Lib/fontgoggles/misc/opentypeTags.py
...
def extractTable(font, tableTag):
    name = font["name"]
    table = font[tableTag]
    ...
    for record in table.table.FeatureList.FeatureRecord:
        data[record.FeatureTag] = entry = dict(characters=set())
                ...
                if subTable.LookupType == 1:
                    for key, value in subTable.mapping.items():
                        for unicode in cmap.get(key, set()):
                            ...
                            entry["characters"].add(chr(unicode))
...
featureData.update(extractTable(font, "GSUB"))
featureData.update(extractTable(font, "GPOS"))
...
for unicode in font.getBestCmap():
    text = chr(unicode).strip()
    if not text:
        continue
    rangeName = customRangeNames.get(text, getRangeName(unicode))
